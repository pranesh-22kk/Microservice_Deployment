...
class FogOrchestrationEnv(gym.Env):
    def __init__(
        ...
        self.node_type = self.np_random.integers(low=0, high=2, size=n_nodes)
        ...
        self.lat_val = self.np_random.integers(low=4, high=10, size=n_nodes)
        self.lat_val[self.idx_fixed] = self.np_random.integers(low=0, high=5, size=self.lat_val[self.idx_fixed].shape)

        # creating the array containing the type of services dewployable in the nodes
        self.serv_type = self.np_random.integers(low=0, high=4, size=n_nodes)
    ...
    def upt_latency(self):
        self.lat_val[~self.idx_fixed] = self.np_random.integers(low=4, high=10, size=self.lat_val[~self.idx_fixed].shape)
        self.lat_val[self.idx_fixed] = self.np_random.integers(low=0, high=5, size=self.lat_val[self.idx_fixed].shape)
    ...
    def service_generator(self) -> dict:
        ...
        if n < 0.1:
            d["id"] = "FVE" + str(self.np_random.integers(low=1, high=101))
            ...
            d["latency"] = self.np_random.integers(low=3, high=8)
        elif n < 0.3:
            d["id"] = "SDP" + str(self.np_random.integers(low=1, high=101))
            ...
            d["latency"] = self.np_random.integers(low=2, high=6)
            d["base"] = "FVE000"
        elif n < 0.6:
            d["id"] = "APP" + str(self.np_random.integers(low=1, high=101))
            ...
            d["id"] = "LAF" + str(self.np_random.integers(low=1, high=101))
