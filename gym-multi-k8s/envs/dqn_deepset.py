# Monkey patch get_obs_shape to support multi-dimensional Box
import stable_baselines3.common.preprocessing as sb3_preprocessing

original_get_obs_shape = sb3_preprocessing.get_obs_shape

def patched_get_obs_shape(observation_space):
    return observation_space.shape

sb3_preprocessing.get_obs_shape = patched_get_obs_shape
...
# Custom ReplayBuffer to handle multi-dimensional observations
...
class Buffer:
    def __init__(self, buffer_size: int, observation_space: spaces.Space, action_space: spaces.Space, device: Union[str, torch.device] = "auto", n_envs: int = 1):
    ...
    def _normalize_obs(self, obs: np.ndarray, env: Optional[VecNormalize] = None) -> np.ndarray:
        if env is not None:
...
class CustomReplayBuffer(Buffer):
    def __init__(self, buffer_size, observation_space, action_space, device="auto", n_envs=1, optimize_memory_usage=False, handle_timeout_termination=True):
        Buffer.__init__(self, buffer_size, observation_space, action_space, device, n_envs=n_envs)
        ...
        self.actions = np.zeros((self.buffer_size, self.n_envs, action_dim), dtype=self.action_space.dtype)
    ...
    def sample(self, batch_size: int, env: Optional[VecNormalize] = None) -> ReplayBufferSamples:
        if not self.full and self.pos == 0:
            raise ValueError("No samples available in the replay buffer")
        if self.full:
            batch_inds = np.random.randint(0, self.buffer_size, size=batch_size)
        else:
            batch_inds = np.random.randint(0, self.pos, size=batch_size)
        obs = self._normalize_obs(self.observations[batch_inds], env)
        next_obs = self._normalize_obs(self.next_observations[batch_inds] if self.next_observations is not None else self.observations[(batch_inds + 1) % self.buffer_size], env)
